...
def from_example_list(args, ex_list, device='cpu', train=True):
    ...
    # input_ids : List[List[int]]: the id of words of sentences after padding
    batch.input_ids = torch.tensor(input_ids, dtype=torch.long, device=device)
    batch.lengths = input_lens
    # input_lens : List[Int]: length of each sentence
    ...
    if train:
        ...
        # tag_ids：List[List[Int]: the id of tag of sentence after padding]
        tag_mask = [[1] * len(ex.tag_id) + [0] * (max_tag_lens - len(ex.tag_id)) for ex in ex_list]
        # tag_mask List[List[Int]]: [1] * length of sentence + [0] * length of pads
        ...
        batch.labels = None
        batch.tag_ids = None
        batch.tag_mask = torch.tensor([[1] * len(ex.input_idx) + [0] * (max_len - len(ex.input_idx)) for ex in ex_list],
