...
class transformer(nn.Module):
    ...
    def forward(self, batch, mode='train'):
        ...
        if mode == 'train':
            ...
            probs_logits = torch.empty((batchsize, max_len, self.config.num_tags)).to(self.device)  # probs_logits = [N, L, num_tags] 用来存放接下来解码的输出 
            ...
            for i in range(max_len):
                ...
                output = self.transformer.decoder(tgt=cur_tgt, memory=memory, tgt_key_padding_mask=cur_padding_mask,
                                                  memory_key_padding_mask=padding_mask)  # output = [N, (i+1), d_model]
